...
try {
    ...
    if (init('action') == 'setPriorityFromDesign') { /* POSTITDESIGN_PRIORITY_OUTLINE_AJAX_V1 */ $eqLogic_id = intval(init('eqLogic_id')); $priority = strtolower(trim((string)init('priority', 'normal'))); $allowed = array('normal', 'important', 'urgent'); if ($eqLogic_id <= 0) { throw new Exception('{{Post-it invalide}}'); } if (!in_array($priority, $allowed, true)) { throw new Exception('{{Priorite invalide}}'); } $eqLogic = eqLogic::byId($eqLogic_id); if (!is_object($eqLogic) || $eqLogic->getEqType_name() != 'postitdesign') { throw new Exception('{{Equipement invalide pour Post-it Design}}'); } $eqLogic->setConfiguration('postit_priority', $priority); $eqLogic->save(); if (method_exists($eqLogic, 'emptyCacheWidget')) { $eqLogic->emptyCacheWidget(); } ajax::success(array('ok' => true, 'eqLogic_id' => $eqLogic->getId(), 'priority' => $priority)); } if (init('action') == 'setColorFromDesign') { /* POSTITDESIGN_QUICK_COLOR_OPTIONS_AJAX_V1 */ $eqLogic_id = intval(init('eqLogic_id')); $color = trim((string)init('color', '#fff4a8')); $allowed = array('#fff4a8', '#ffd6e7', '#dbeafe', '#dcfce7', '#ffffff', '#fed7aa', '#e9d5ff', '#e5e7eb'); if ($eqLogic_id <= 0) { throw new Exception('{{Post-it invalide}}'); } if (!in_array(strtolower($color), $allowed, true)) { throw new Exception('{{Couleur invalide}}'); } $eqLogic = eqLogic::byId($eqLogic_id); if (!is_object($eqLogic) || $eqLogic->getEqType_name() != 'postitdesign') { throw new Exception('{{Equipement invalide pour Post-it Design}}'); } $eqLogic->setConfiguration('postit_color', strtolower($color)); $eqLogic->save(); if (method_exists($eqLogic, 'emptyCacheWidget')) { $eqLogic->emptyCacheWidget(); } ajax::success(array('ok' => true, 'eqLogic_id' => $eqLogic->getId(), 'color' => strtolower($color))); } if (init('action') == 'setTitleFromDesign') { /* POSTITDESIGN_TITLE_EDIT_FROM_DESIGN_AJAX_V1 */
        $eqLogic_id = intval(init('eqLogic_id'));
